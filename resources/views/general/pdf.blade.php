...
<html>
<head>
  <title>Animaciòn y Recreaciòn 10</title>
  <style type="text/css">
...
{{--   <div><b>Fecha de Registro: </b>{{$querys_pdf->fecha}}</div>
  ...
  <div><b>Disciplina: </b>{{$querys_pdf->diciplina}}</div>
  <div><b>Desempeño: </b>{{$querys_pdf->desempeño}}</div>
  <div><b>Categoria: </b>{{$querys_pdf->categoria}}</div>
  <div><b>Nombre y Apellido: </b>{{$querys_pdf->nombre}}</div>
  <div><b>Cedula: </b>{{$querys_pdf->cedula}}</div>
  <div><b>Lugar de Nacimiento: </b>{{$querys_pdf->lugar}}</div>
  <div><b>Fecha de Nacimiento: </b>{{$querys_pdf->fecha_n}}</div>
  <div><b>Edad: </b>{{$querys_pdf->edad}}</div>
  ...
  <div><b>Participante: </b>{{$querys_pdf->partcipante}}</div>
  <div><b>Tiempo de Trayectoria: </b>{{$querys_pdf->trayectoria}}</div>
  <div><b>Nombre de la Asociaciòn: </b>{{$querys_pdf->asociacion}}</div>
  <div><b>Especificaciòn de la Actividad que Realiza: </b>{{$querys_pdf->actividad}} </div> --}}
